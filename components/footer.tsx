...
export function Footer() {
  ...
  return (
    <footer ref={ref} className="border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
      <div className="max-w-6xl mx-auto px-4 py-16">
        ...
        >
          ...
          <div className="col-span-2 md:col-span-1">
            <a href="#" className="flex items-center gap-2 mb-4">
              <div className="w-8 h-8 rounded-lg bg-zinc-950 dark:bg-white flex items-center justify-center">
                <span className="text-white dark:text-zinc-950 font-bold text-sm">A</span>
              </div>
              <span className="font-semibold text-zinc-950 dark:text-white">Apex</span>
            </a>
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
              ...
              <span className="text-xs text-zinc-600 dark:text-zinc-400">All Systems Operational</span>
            </div>
          </div>
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 className="text-sm font-semibold text-zinc-950 dark:text-white mb-4">{title}</h4>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-sm text-zinc-500 hover:text-zinc-950 dark:hover:text-white transition-colors">
                      {link}
                    </a>
                  </li>
                ))}
